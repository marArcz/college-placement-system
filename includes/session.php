...
const USER_SESSION_NAME = "stii_cps_student_session";
...
class Session
{
    ...
    public static function getUser(PDO $pdo)
    {
        ...
        $query = $pdo->prepare("SELECT students.*,users.email FROM students INNER JOIN users ON students.user_id = users.id WHERE students.user_id = ?");
