...
const USER_SESSION_NAME = "stii_cps_admin_session";
...
class Session
{
    ...
    public static function getUser(PDO $pdo)
    {
        ...
        $query = $pdo->prepare("SELECT admins.*,users.email,users.password FROM admins INNER JOIN users ON admins.user_id = users.id WHERE admins.user_id = ?");
